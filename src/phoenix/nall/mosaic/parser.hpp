              ...
              for(unsigned tileX = 0; tileX < ctx.tileWidth; tileX++) {
                ...
                for(unsigned blockY = 0; blockY < ctx.blockHeight; blockY++) {
                  for(unsigned blockX = 0; blockX < ctx.blockWidth; blockX++) {
                    if(import) {
                      unsigned palette = 0;
                      for(unsigned n = 0; n < ctx.depth; n++, blockOffset++) {
                        palette |= stream.read(offset + ctx.block(blockOffset, blockOffset)) << n;
                      }
                      ...
                      for(unsigned n = 0; n < ctx.depth; n++, blockOffset++) {
                        stream.write(offset + ctx.block(blockOffset, blockOffset), palette & 1);
                        palette >>= 1;
                      }
                    }
                  }  //blockX
